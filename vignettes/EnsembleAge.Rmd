...
- **All previously published mouse clocks** (e.g., Petkovich, Horvath, Stubbs, etc.)
- **All previously published universal clocks** (e.g., Universal Methylation Age)
- **Our novel ensemble clocks** that combine multiple validated approaches

### Comparison to Existing Solutions

#### vs. methyclock and Similar Packages
While packages like `methyclock` provide individual clock implementations, EnsembleAge offers:

1. **Ensemble Robustness**: Rather than relying on single clocks that often yield inconsistent results, EnsembleAge integrates predictions from multiple penalized models, providing more reliable age estimates.

2. **Comprehensive Coverage**: Includes both published clocks and our novel ensemble methods in a single, unified interface.

3. **Cross-Species Validation**: Our clocks are specifically validated across multiple mammalian species, addressing the translational gap between mouse models and human studies.

4. **Intervention Detection**: Empirical evaluations demonstrate that EnsembleAge outperforms existing clocks in detecting both pro-aging and rejuvenating interventions.
