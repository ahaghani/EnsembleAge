...
**Abstract**: Several widely used epigenetic clocks have been developed for mice and other species, but a persistent challenge remains: different mouse clocks often yield inconsistent results. To address this limitation in robustness, we present EnsembleAge, a suite of ensemble-based epigenetic clocks. Leveraging data from over 200 perturbation experiments across multiple tissues, EnsembleAge integrates predictions from multiple penalized models. Empirical evaluations demonstrate that EnsembleAge outperforms existing clocks in detecting both pro-aging and rejuvenating interventions. Furthermore, we introduce EnsembleAge HumanMouse, an extension that enables cross-species analyses, facilitating translational research between mouse models and human studies. Together, these advances underscore the potential of EnsembleAge as a robust tool for identifying and validating interventions that modulate biological aging.

## Motivation and Comparison to Existing Packages

### Novel Contributions and Scientific Innovation

EnsembleAge introduces several novel epigenetic clocks and addresses critical limitations in existing approaches. Our work leverages data from over 200 perturbation experiments across multiple tissues to develop robust ensemble-based epigenetic clocks that significantly outperform existing solutions.

#### Novel Clock Development
- **EnsembleAge Mouse Clocks**: Novel mouse-specific epigenetic clocks leveraging mammalian methylation arrays
- **EnsembleAge Universal Clocks**: Cross-species clocks for translational research
- **EnsembleAge HumanMouse**: Extension enabling cross-species analyses between mouse models and human studies

#### Comprehensive Clock Collection
Unlike existing packages that focus on individual clocks, EnsembleAge provides access to:
- **All previously published mouse clocks** (e.g., Elastic Net Clocks, Lifespan Uber Clock, etc.)
- **All previously published universal clocks** (e.g., Universal Methylation Age, etc.)
- **Our novel ensemble clocks** that combine multiple validated approaches

### Comparison to Existing Solutions

#### vs. methyclock and Similar Packages
While packages like `methyclock` are primarily focused on human epigenetic clocks, EnsembleAge offers specialized functionality for mammalian arrays and mouse clocks:

1. **Mammalian Array Specialization**: Unlike `methyclock` which focuses on human arrays (EPIC/450k), EnsembleAge is specifically designed for mammalian methylation arrays (Mammal320k, Mammal40k) and provides comprehensive mouse clock implementations.

2. **Mouse Clock Focus**: While `methyclock` provides human-centric clocks, EnsembleAge specializes in mouse epigenetic clocks, including all previously published mouse clocks and our novel ensemble approaches.

3. **Ensemble Robustness**: Rather than relying on single clocks that often yield inconsistent results, EnsembleAge integrates predictions from multiple penalized models, providing more reliable age estimates specifically for mouse models.

4. **Comprehensive Coverage**: Includes both published mouse clocks and our novel ensemble methods in a single, unified interface, while also providing universal clocks for cross-species comparisons.

5. **Cross-Species Validation**: Our clocks are specifically validated across multiple mammalian species, addressing the translational gap between mouse models and human studies.

6. **Intervention Detection**: Empirical evaluations demonstrate that EnsembleAge outperforms existing clocks in detecting both pro-aging and rejuvenating interventions in mouse models.
...
2. **Ensemble Approach**: Combines multiple validated clock methods to provide more robust age predictions, addressing the inconsistency problem in existing mouse clocks.
...
Our ensemble approach addresses a persistent challenge in the field: different mouse clocks often yield inconsistent results. By integrating predictions from multiple models trained on extensive perturbation data, EnsembleAge provides more reliable age estimates and better detection of aging interventions, making it a robust tool for identifying and validating interventions that modulate biological aging.
